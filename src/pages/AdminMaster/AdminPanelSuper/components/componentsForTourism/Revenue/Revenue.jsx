...
const Revenue = () => {
	...
	const handleDateChange = (dates) => {
		if (dates && dates[0] && dates[1]) {
			...
			const totalRevenue = filtered.reduce(
				(sum, item) => sum + item.totalPackagePrice,
				0
			);
			...
			setRevenue(
				revenueData.reduce((sum, item) => sum + item.totalPackagePrice, 0)
			); // Reset total revenue
		}
	};
	...
	const data = filteredData?.map((item, index) => {
		return {
			No: index + 1,
			agentCode: item._doc.agentCode,
			agentId: item.agentId,
			agentName: item.agentName,
			packageName: item._doc.packageId.name,
			destination: item._doc.packageId.destination,
			totalPackagePrice: item._doc.totalPackagePrice,
			createdAt: item._doc.createdAt, // Include createdAt if needed for debugging
		};
	});
