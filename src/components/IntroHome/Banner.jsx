...
const Banner = () => {
    ...
    const handleToursAndTravelsClick = () => {
        const url = token && loggedInUser
            ? `/travel-app/?token=${encodeURIComponent(token)}&userName=${encodeURIComponent(JSON.parse(loggedInUser).fullName)}&email=${encodeURIComponent(JSON.parse(loggedInUser).email)}&userId=${encodeURIComponent(JSON.parse(loggedInUser)._id)}&phoneNumber=${encodeURIComponent(JSON.parse(loggedInUser).phoneNumber)}`
            : '/login';
        navigate(url);
    };
